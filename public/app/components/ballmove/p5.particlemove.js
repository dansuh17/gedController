 ...
 * by Young Jin Park
 */
/* global $:true, width:true, height:true */
var particles;
...
function setup() {
  ...
  for (i = 0; i < totalcount; i++) {
    ...
    n = Math.floor(Math.random(0, 6)); // randomize the faces to draw
    particle.setSpeed(Math.random(0, 80), Math.random(0, 50));
...
function updateParticle(p) {
  ...
  var ax = (0.002 * rx) - (0.1 * p.velocity.x) + Math.random(-2, 2);
  var ay = (0.002 * ry) - (0.1 * p.velocity.y) + Math.random(-2, 2);
